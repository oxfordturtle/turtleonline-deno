...
export const readUser = async (
  usernameOrFields: string | Partial<User>
): Promise<Maybe<User>> => {
  const fields =
    typeof usernameOrFields === "string"
      ? { username: usernameOrFields }
      : usernameOrFields;
...
export const readUsers = async (
  ...
  const users = await asafelyOptional<User[]>(async () =>
    JSON.parse(await Deno.readTextFile(usersPath))
  );
...
const matches = (user: User, fields: Partial<User>): boolean =>
  ...
  (fields.emailConfirmed === undefined ||
    fields.emailConfirmed === user.emailConfirmed) &&
  (fields.password === undefined || fields.password === user.password) &&
  (fields.lastLoginDate === undefined ||
    fields.lastLoginDate === user.lastLoginDate) &&
  (fields.token === undefined || fields.token === user.token) &&
  (fields.tokenExpires === undefined ||
    fields.tokenExpires === user.tokenExpires) &&
  ...
  (fields.schoolPostcode === undefined ||
    fields.schoolPostcode === user.schoolPostcode) &&
  (fields.guardian === undefined || fields.guardian === user.guardian) &&
  (fields.admin === undefined || fields.admin === user.admin) &&
  (fields.receivingEmails === undefined ||
    fields.receivingEmails === user.receivingEmails) &&
  (fields.systemSettings === undefined ||
    fields.systemSettings === user.systemSettings);
