...
import { FormInput } from "../../layout/form.tsx"
import Feedback, { FeedbackProps } from "../../layout/feedback.tsx"
...
const passwordResponse = (requestParams: RequestParams, user: User, feedback?: FeedbackProps): Promise<Response> =>
...
const handleForm = async (requestParams: RequestParams, user: User, imp: Imp): Promise<Response> => {
  ...
  if (requestParams.formData === undefined) {
    return passwordResponse(requestParams, user, { ok: false, message: "You didn't submit any form data." })
  }

  // get form fields
  const currentPassword = getFormField("currentPassword", requestParams.formData) ?? ""
  const newPassword = getFormField("newPassword", requestParams.formData) ?? ""
  const repeatPassword = getFormField("repeatPassword", requestParams.formData) ?? ""

  // validate form fields
  if (newPassword !== repeatPassword) {
    return passwordResponse(requestParams, user, { ok: false, message: "New passwords don't match." })
  }
  ...
  return (await imp.updateUser(user.username, userDetails))
    ? passwordResponse(requestParams, { ...user, ...userDetails }, { ok: true, message: "Your password has been updated." })
    : passwordResponse(requestParams, user, {
        ok: false,
...
const main = (feedback?: FeedbackProps): JSX.Element => (
  <>
    ...
    <div className="form">
      <form method="post">
        <FormInput label="Current Password" id="currentPassword" type="password" required={true} />
        <div className="columns">
          <div className="column">
            <FormInput label="New Password" id="newPassword" type="password" required={true} />
          </div>
          <div className="column">
            <FormInput label="Repeat Password" id="repeatPassword" type="password" required={true} />
          </div>
        </div>
