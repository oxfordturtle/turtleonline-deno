...
import { FormInput, FormSelect, FormOption } from "../layout/form.tsx"
...
const handleForm = async (requestParams: RequestParams, imp: Imp): Promise<Response> => {
  ...
  if (await imp.readUser(username)) return registerResponse(requestParams, { ok: false, message: "Username is already taken." })
  ...
  if (accountType === 2 && !guardian) return registerResponse(requestParams, { ok: false, message: "Full name of parent/guardian is required." })
  ...
  return (await imp.createUser(user))
    ...
    : registerResponse(requestParams, { ok: false, message: "Something went wrong creating your account. Please try again." })
}
