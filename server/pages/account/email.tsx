...
const handleForm = async (
  ...
  if (requestParams.formData === undefined) {
    return emailResponse(requestParams, user, {
      ok: false,
  ...
  const receivingEmailsString =
    getFormField("receivingEmails", requestParams.formData) ?? "";
  const receivingEmailsNumber = parseInt(receivingEmailsString);
  const receivingEmails = isNaN(receivingEmailsNumber)
    ? undefined
    : Boolean(receivingEmailsNumber);

  // create user details
  const userDetails: Partial<User> = {};
  if (email) userDetails.email = email;
  if (receivingEmails !== undefined)
    userDetails.receivingEmails = receivingEmails;
